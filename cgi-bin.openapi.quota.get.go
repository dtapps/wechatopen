...
func (c *Client) CgiBinOpenapiQuotaGet(ctx context.Context, componentAccessToken string, cgiPath string, notMustParams ...gorequest.Params) (*cgiBinOpenapiQuotaGetResult, error) {
	...
	// 请求
	request, err := c.request(ctx, apiUrl+"/cgi-bin/openapi/quota/get?access_token="+componentAccessToken, params, http.MethodPost)
	if err != nil {
		return newcgiBinOpenapiQuotaGetResult(cgiBinOpenapiQuotaGetResponse{}, request.ResponseBody, request), err
	}
	// 定义
